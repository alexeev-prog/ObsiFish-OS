...
C=i386-elf-gcc
LD=i386-elf-ld
DISKIMG_NAME=obsifishos_floppy_i386_0.13.24.img
...
build:
	...
	$(C) -g -m32 -nostdlib -nostdinc -fno-builtin -fno-stack-protector -nostartfiles -nodefaultlibs -Wall -Wextra -Werror -ffreestanding -c src/drivers/audio.c -o bin/audio.o
	...
	$(LD) -o bin/kernel.bin -Ttext 0x1000 bin/kernel_entry.o bin/kernel.o bin/audio.o bin/interrupt.o bin/keyboard.o bin/screen.o bin/idt.o bin/isr.o bin/ports.o bin/timer.o bin/mem.o bin/string.o bin/utils.o --oformat binary
	cat bin/bootsector.bin bin/kernel.bin > bin/obsifishos.bin
...
run_bin:
	qemu-system-i386 -fda bin/obsifishos.bin

diskimg:
	mkdir -p diskimgs
	dd if=/dev/zero of=diskimgs/$(DISKIMG_NAME) bs=1024 count=1440
	dd if=$(BIN_DIR)/obsifishos.bin of=diskimgs/$(DISKIMG_NAME) conv=notrunc
